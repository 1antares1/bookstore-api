...
//#region tokens
import { ITuple } from "../../models/tuple";
//#endregion
...
export class UtilService implements IUtilService {
    ...
    public getMonths(qtyFrom?: number): Array<ITuple<any>> {
      let months: string[];
      ...
      if (moment) {
          ...
          if (months.length) {
              if (!isNaN(qtyFrom)) {
                  // better than splice, it preserves index (+performance than moment().month())
    ...
    public paddy(value: string, padCount: number, character?: string): string {
        const padChar = (character ? character : "0");
        const pad = [1 + padCount].join(padChar);
    ...
    //#region dom
    public findElementPos(element: HTMLElement): number {
        let top = 0;
